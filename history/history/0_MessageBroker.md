 ...
 * ✅ 완전 구현 완료 (2025-11-11)
 * - UC-20: 설비 상태 보고 저장
 * - UC-21: 최근 보고 시각 조회 (타임아웃 감지용)
 * - 알림 발송 내역 저장
 * 
 * Database Schema (DB_MONITOR):
 * - equipment_status: 설비 상태 보고 이력
 * - notification_log: 알림 발송 내역
 * 
 * Implementation Details:
 * - JDBC PreparedStatement 사용 (SQL Injection 방지)
 * - Connection Pool 지원 (HikariCP 권장)
 * - 인덱스 최적화: equipment_id, reported_at
 */
public class MonitorRepositoryImpl implements IMonitorRepository {
    ...
    public void saveStatus(EquipmentStatusReport report) {
        ...
        try (Connection conn = dataSource.getConnection();
            ...
            throw new RuntimeException("Failed to save equipment status", e);
        }
    }
    ...
    public Date findLastReportTime(String equipmentId) {
        ...
        try (Connection conn = dataSource.getConnection();
            ...
            throw new RuntimeException("Failed to find last report time", e);
        }
    }
    ...
    public void saveNotificationLog(String equipmentId, String alertMessage) {
        ...
        try (Connection conn = dataSource.getConnection();
            ...
            throw new RuntimeException("Failed to save notification log", e);
        }
    }
    
    // 추가 유틸리티 메서드
    public List<String> findAllEquipmentIds() {
        // 하드코딩된 설비 목록 대체용 - DB에서 동적 조회
    ...
    public int countFaultsSince(String equipmentId, Date since) {
        ...
        try (Connection conn = dataSource.getConnection();
            ...
            throw new RuntimeException("Failed to count faults", e);
        }
    }
}
